...
export default function OpenPositions() {
  ...
  const { openingPositionsCount, updateOpeningPositionsCount } = useGlobalCount()
  ...
  useEffect(() => {
    if (openingPositionsCount === openPositions?.length || 0) return
    if (!openPositions) {
      updateOpeningPositionsCount(0)
      return
    }
    updateOpeningPositionsCount(openPositions?.length)
  }, [openPositions])
