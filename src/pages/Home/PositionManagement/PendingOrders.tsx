...
export default function PendingOrders() {
  ...
  const { pendingOrdersCount, updatePendingOrdersCount } = useGlobalCount()
  ...
  useEffect(() => {
    if (pendingOrdersCount === pendingOrders?.length || 0) return
    if (!pendingOrders) {
      updatePendingOrdersCount(0)
      return
    }
    updatePendingOrdersCount(pendingOrders?.length)
