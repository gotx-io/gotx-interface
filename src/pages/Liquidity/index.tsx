    ...
    <>
      ...
      <Box sx={{ width: '100%', maxWidth: 1000, mx: 'auto' }}>
        ...
        <Type.Body color="neutral5" mb={24}>
          <Trans>Purchase GLI tokens to earn USDC from trading fees.</Trans>
        </Type.Body>
  ...
  return (
    <Box sx={{ bg: 'background2', borderRadius: 'sm', p: 3 }}>
      <Flex sx={{ alignItems: 'center', gap: 3 }}>
        <TokenWrapper symbol="GLI" size={48} hasText={false} />
        <Box>
          <Type.Body sx={{ display: 'block' }}>GLI</Type.Body>
          <Type.Caption color="neutral5">
            <Trans>GotX Liquidity Index</Trans>
          </Type.Caption>
        </Box>
      </Flex>
      <Divider my={3} />
...
const TABS = [
  { label: 'Buy GLI', value: 'buy' },
  { label: 'Sell GLI', value: 'sell' },
]
...
function Form() {
  ...
  const { isTokenAllowanceEnough, approving, approveToken } = useERC20Approval({
    token: CONTRACT_ADDRESSES[DEFAULT_CHAIN_ID][CONTRACT_KEYS.USDC],
...
function USDCToken() {
  ...
  return (
    <Flex sx={{ flexShrink: 0, flexDirection: 'column', alignItems: 'end' }}>
      <Type.Body mb={3}>Balance: {USDC ? formatNumber(USDC.num) + ' USDC' : '--'}</Type.Body>
      <Flex sx={{ gap: 2, height: 40, alignItems: 'center' }}>
        <TokenWrapper symbol="USDC" />
      </Flex>
    </Flex>
  )
}
function BLIToken() {
  const { GLI } = useBalancesStore((state) => state.balances)
  return (
    <Flex sx={{ flexShrink: 0, flexDirection: 'column', alignItems: 'end' }}>
      <Type.Body mb={3}>Balance: {GLI ? formatNumber(GLI.num) + ' GLI' : '--'}</Type.Body>
      <Flex sx={{ gap: 2, height: 40, alignItems: 'center' }}>
        <TokenWrapper symbol="GLI" />
      </Flex>
    </Flex>
  )
}
